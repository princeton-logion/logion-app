...
1. Select a Levenshtein distance from the drop-down menu to the right of the model selection menu. We recommend starting with a Levenshtein distance of 1. For more on what is Levenshtein distance, see our (technical explainer page on the topic)[../explainers/lev-dist.md].
...
1. Click the blue "Detect Errors" button below the text area. Note the error detection process can take several minutes depending on one's local hardware. To read more on how hardware affects error detection processing speed, click [here](https://princeton-logion.github.io/logion-app/hardware/).
