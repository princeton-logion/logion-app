...
Logion's word and character prediction feature predicts words that may be missing in a text. The feature works best for filling gaps that likely span no more than one word.
...
1. **Type** your text into the text area. Use "**-**" to represent any missing words or characters (depending on which feature you are using). For best results, enter only text comprised of Greek characters and periods.
...
Logion displays results below the blue **Predict** button.
