...
def prediction_function(text: str, model: str, tokenizer: str, device, window_size: int=512, overlap: int=128, num_predictions: int=5):
    ...
    for i in range(0, num_tokens, window_size - overlap):
        ...
        chunk_inputs = tokenizer(chunk, return_tensors='pt', return_attention_mask=True, add_special_tokens=True, truncation=True, max_length=512)
        ...
        masked_indices = [i for i, token_id in enumerate(chunk_inputs['input_ids'][0]) if token_id == tokenizer.mask_token_id]
