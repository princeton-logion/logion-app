...
def detection_function(text: str, model: logion_class.Logion, tokenizer, device, chunk_size: int = 500, lev: int = 1, no_beam=False):
    """
    Masked langauge modeling for detecting and suggesting corrections to potential errorneous words in input text.
    ...
    Returns:
        ...
        *** list indices correspond to order of dict key entires ***
    """
    ...
    while chunk_start < num_tokens_full:
        ...
        # "." = end of chunk
        while chunk_end > chunk_start and tokenizer.convert_ids_to_tokens(chunk_tokens[-1]) != ".":
            chunk_end -= 1
            chunk_tokens = tokens_full[chunk_start:chunk_end]
        ...
        for word_ind, word_score in enumerate(word_scores):
            ...
            # seperate tokens into [MASK] token, pre, and post tokens
            ...
            suggestions = model.fill_the_blank_given_transmitted(
                model.display_sentence(tokenizer.convert_ids_to_tokens(pre_tokens)), # pre-text
                model.display_sentence(tokenizer.convert_ids_to_tokens(post_tokens)), # post-text
                model.display_sentence(tokenizer.convert_ids_to_tokens(transmitted_tokens)), # masked word
                transmitted_tokens,
                masked_token_ids,
                max_lev=lev, no_beam=no_beam
            )
        ...
        for word_index, (word, score) in enumerate(zip(words, word_scores)):
            ...
            original_word = model.display_sentence(tokenizer.convert_ids_to_tokens(word))

            chunk_predictions[(original_word, score, global_word_index)] = all_suggestions[word_index]

            first_suggestion_score = all_suggestions[word_index][0][1]
            ccr_value = score / first_suggestion_score if first_suggestion_score != 0 else float('inf')
