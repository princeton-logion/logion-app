...
"""
gbst_mod.py
modification of charformer_pytorch.py from github.com/lucidrains/charformer-pytorch
"""
import math
...
class GBST(nn.Module):
    ...
    # do forward with embeddings rather than token IDs
    def forward_from_embeddings(self, embeddings, mask = None):
        b, n, block_mult, ds_factor, device = *embeddings.shape[:2], self.block_pad_multiple, self.downsample_factor, embeddings.device
        m = next_divisible_length(n, ds_factor)

        target_dtype = next(self.parameters()).dtype
        embeddings = embeddings.to(target_dtype)

        x = self.pos_conv(embeddings)
        ...
        return x, mask
    # orig forward method for token IDs
    def forward(self, x, mask = None):
        x = self.token_emb(x)
        return self.forward_from_embeddings(x, mask)
