...
def test_predict_reproducibility(text, model_and_tokenizer):
    ...
    for (key1, value1), (key2, value2) in zip(final_predictions_1.items(), final_predictions_2.items()):
      assert key1 == key2
...
def test_window_size_and_overlap(model_and_tokenizer):
    ...
    Asserts:
        ...
        2. Different sliding window and overlaps return idential key-value pairs
    """
    model, tokenizer, device = model_and_tokenizer

    text = "Ἐν ἀρχῇ ἦν ὁ [MASK], καὶ ὁ λόγος ἦν πρὸς τὸν θεόν, καὶ θεὸς ἦν ὁ λόγος." * 40

    mask_count = text.count("[MASK]")

    final_predictions_1 = predict.prediction_function(text, model, tokenizer, device, window_size=10, overlap=5)
    final_predictions_2 = predict.prediction_function(text, model, tokenizer, device, window_size=20, overlap=10)
