...
def mock_model_and_tokenizer():
    ...
    mock_tokenizer.encode.return_value = [2, 477, 1774, 629, 95, 4, 12, 453, 95, 1040, 629, 547, 510, 1154, 12, 453, 921, 629, 95, 1040, 14, 3]
    mock_tokenizer.decode.side_effect = lambda ids: "Ἐν ἀρχῇ ἦν ὁ [MASK], καὶ ὁ λόγος ἦν πρὸς τὸν θεόν, καὶ θεὸς ἦν ὁ λόγος."
    mock_tokenizer.mask_token_id = 4  # [MASK] == 4
    mock_tokenizer.convert_ids_to_tokens.side_effect = lambda ids: ["[MASK]" if i == 4 else f"token_{i}" for i in ids]
    
    def mock_tokenizer_call(text, return_tensors, return_attention_mask, add_special_tokens, truncation, max_length):
        return {
            "input_ids": torch.tensor([[2, 477, 1774, 629, 95, 4, 12, 453, 95, 1040, 629, 547, 510, 1154, 12, 453, 921, 629, 95, 1040, 14, 3]]),
    ...
    logits = torch.rand((1, 22, 50000))  # mock logits (batch_size = 1, sequence_length = 22, vocab_size = 50k)
